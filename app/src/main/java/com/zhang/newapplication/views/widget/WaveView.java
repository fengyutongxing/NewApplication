...
/**
 * Created by zhang_shuai on 2017/7/26.
 * Del:波浪形状的view
 */
...
public class WaveView extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        double width = 2*Math.PI / getWidth();
        ...
        for (float x = 0; x <= getWidth(); x += 20) {
             ...
             *  width—角速度， 控制正弦周期(单位角度内震动的次数)
             *  z—初相，反映在坐标系上则为图像的左右移动。这里通过不断改变z,达到波浪移动效果
             *  k—偏距，反映在坐标系上则为图像的上移或下移。
             */
            y = (float) (8 * Math.cos(width * x + z) +8);
            y2 = (float) (8 * Math.sin(width * x + z));
